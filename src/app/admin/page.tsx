...
import { dispatchMessagePush } from "@/lib/push-notifications";
...
export default function AdminPage(){
  ...
  useEffect(()=>{if(!selected)return;const ch=supabase.channel(`admin-messages-${selected.id}`).on("postgres_changes",{event:"INSERT",schema:"public",table:"messages",filter:`client_id=eq.${selected.id}`},payload=>setMessages(cur=>cur.some(x=>x.id===payload.new.id)?cur:[...cur,payload.new])).subscribe();return()=>{supabase.removeChannel(ch)}},[selected?.id]);
